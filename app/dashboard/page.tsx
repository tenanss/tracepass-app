...
const PLAN_LIMITS = { starter: 3, business: 20, enterprise: 9999 };
...
export default function DashboardPage() {
  ...
  const handleUpgrade = async (period: 'monthly' | 'yearly') => {
    const priceIds = { monthly: 'price_1T28vyE7LrcUGUCEt5fI9g2t', yearly: 'price_1T28vyE7LrcUGUCEo593b8v5' };
    try {
...
const productData = { 
    ...
    return (
      <div className="min-h-screen bg-white p-10">
        ...
        <div className="grid grid-cols-4 gap-6">
          {products.map((p) => (
            <div key={p.id} className="border p-6 flex flex-col items-center rounded-xl bg-white text-center page-break-inside-avoid">
              <QRCodeSVG value={`${window.location.origin}/product/${p.id}`} size={120} level="H" />
  ...
  return (
    <div className="min-h-screen bg-[#f1f3f5] font-sans text-slate-900">
      <nav className="bg-white/80 border-b border-slate-200 sticky top-0 z-50 px-8 py-4 flex justify-between items-center">
      ...
      <main className="max-w-7xl mx-auto p-8 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          ...
          <button onClick={() => setIsPrintMode(true)} className="md:col-span-1 bg-slate-800 p-8 rounded-[2.5rem] text-white shadow-xl min-h-[180px] flex flex-col justify-between">
            <span className="text-3xl">⎙</span>
            <span className="text-sm font-black uppercase leading-tight">Stampa Etichette QR</span>
          </button>
          ...
          <div className="bg-[#f8f9fa] w-full max-w-2xl rounded-[3rem] p-10 shadow-2xl my-auto">
            ...
            <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                ...
                <input required className="w-full p-5 rounded-2xl bg-white border border-slate-200 text-sm font-bold shadow-sm" value={newProduct.name} onChange={(e) => setNewProduct({...newProduct, name: e.target.value})} />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block ml-4">Origine</label>
                <input className="w-full p-5 rounded-2xl bg-white border border-slate-200 text-sm font-bold shadow-sm" value={newProduct.origin} onChange={(e) => setNewProduct({...newProduct, origin: e.target.value})} />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block ml-4">Carbon Footprint</label>
                <input className="w-full p-5 rounded-2xl bg-white border border-slate-200 text-sm font-bold shadow-sm" value={newProduct.carbon_footprint} onChange={(e) => setNewProduct({...newProduct, carbon_footprint: e.target.value})} />
              </div>
              <div className="md:col-span-2">
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block ml-4">Materiali Principali</label>
                <input className="w-full p-5 rounded-2xl bg-white border border-slate-200 text-sm font-bold shadow-sm" value={newProduct.material_composition} onChange={(e) => setNewProduct({...newProduct, material_composition: e.target.value})} />
              </div>
              <div className="md:col-span-2">
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block ml-4">Istruzioni Riciclo</label>
                <textarea className="w-full p-5 rounded-2xl bg-white border border-slate-200 text-sm font-bold shadow-sm min-h-[80px]" value={newProduct.recycling_instructions} onChange={(e) => setNewProduct({...newProduct, recycling_instructions: e.target.value})} />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block ml-4">Sostanze (REACH)</label>
                <input className="w-full p-5 rounded-2xl bg-white border border-slate-200 text-sm font-bold shadow-sm" value={newProduct.substances_reach} onChange={(e) => setNewProduct({...newProduct, substances_reach: e.target.value})} />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block ml-4">Repair Score</label>
                <select className="w-full p-5 rounded-2xl bg-white border border-slate-200 text-sm font-bold shadow-sm" value={newProduct.repair_score} onChange={(e) => setNewProduct({...newProduct, repair_score: e.target.value})}>
              ...
              <div className="md:col-span-2 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm mt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="text-[9px] font-black uppercase text-slate-400 block mb-2 ml-2">Video Tutorial (URL)</label>
